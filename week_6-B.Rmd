...
```{r}
tibble(urn_red = seq(1, 1000, by = 1)) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100, size = 25, prob = ./1000))) %>% 
  unnest(paddle_red) %>%
  ggplot(aes(paddle_red, urn_red)) +
  geom_point(alpha = .1)
...
```{r}
tibble(urn_red = seq(1, 1000, by = 1)) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100, size = 20, prob = ./1000))) %>% 
  unnest(paddle_red) %>%
  mutate(res = ifelse(paddle_red > 20, TRUE, FALSE)) %>% 
  summarise(final = weighted.mean(res, prob)) 
